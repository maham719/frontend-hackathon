...
import { getAgentTicketsService } from "../../features/tickets/services/ticket.service.js";
...
const AllTickets = () => {
  ...
  const fetchTickets = async () => {
    try {
      setLoading(true);
      setError("");
      const agentTickets = await getAgentTicketsService();
      setTickets(agentTickets);
    } catch (fetchError) {
      console.error("FETCH AGENT TICKETS ERROR:", fetchError);
      console.error("STATUS:", fetchError.response?.status);
      console.error("DATA:", fetchError.response?.data);
      setError(fetchError.response?.data?.message || "Unable to load your assigned tickets right now.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <section className={`rounded-[22px] border p-5 shadow-[0_20px_45px_rgba(67,47,92,0.12)] sm:p-7 ${panel}`}>
      <div className="mb-6 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        ...
        <button
          ...
          className={`rounded-xl border px-4 py-2 text-sm font-semibold ${control}`}
        >
          Refresh
        </button>
      </div>

      <div className="mb-5 grid gap-3 lg:grid-cols-[minmax(220px,1fr)_repeat(3,minmax(140px,0.25fr))]">
        <label className="relative block">
          ...
          <input
            ...
            className={`w-full rounded-xl border py-2.5 pl-10 pr-3 text-sm outline-none focus:border-[#8d5fe5] ${control}`}
          />
        </label>

        {[
          ["Status", status, setStatus, ["Open", "In Progress", "Resolved", "Closed"]],
          ["Priority", priority, setPriority, ["Low", "Medium", "High", "Urgent"]],
          ...
          <select
            ...
            className={`rounded-xl border px-3 py-2.5 text-sm outline-none focus:border-[#8d5fe5] ${control}`}
          >
      ...
      {loading ? (
        <div className={`flex items-center justify-center gap-3 px-6 py-16 text-sm ${muted}`}>
          <Clock3 size={18} /> Loading assigned tickets...
        </div>
      ) : error ? (
        ...
        <div className={`flex flex-col items-center px-6 py-16 text-center ${muted}`}>
          <Ticket size={26} />
          <p className={`mt-3 text-lg font-semibold ${heading}`}>No assigned tickets found</p>
          <p className="mt-1 text-sm">Try changing your search or filters.</p>
        </div>
      ) : (
        <div className={`overflow-hidden rounded-[18px] border ${isDark ? "border-[#2b3548]" : "border-[#e7dff3]"}`}>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[900px] text-left text-sm">
              <thead className={isDark ? "bg-[#171f2d]" : "bg-[#efe8f8]"}>
                <tr>
                  {[
                    ...
                    <th key={label} className={`px-4 py-3 text-[10px] font-semibold uppercase tracking-[0.14em] ${muted}`}>
                      {label}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className={`divide-y ${isDark ? "divide-[#2b3548]" : "divide-[#e7dff3]"}`}>
                {visibleTickets.map((ticket) => {
                  const ticketStatus = formatLabel(ticket.status, statusLabels);
                  const ticketPriority = formatLabel(ticket.priority, priorityLabels);

                  return (
                    <tr
                      key={ticket._id}
                      onClick={() => navigate(`/agent-dashboard/tickets/${ticket._id}`)}
                      className={`cursor-pointer transition-colors ${isDark ? "hover:bg-[#172335]" : "hover:bg-[#f0e9f8]"}`}
                    >
                      ...
                      <td className={`px-4 py-4 ${heading}`}>
                        {ticket.customer?.username || ticket.customer?.email || "Unassigned"}
                      </td>

                      <td className={`max-w-[240px] truncate px-4 py-4 font-medium ${heading}`}>
                      ...
                      <td className="px-4 py-4">
                        <span className={`rounded-full px-2.5 py-1 text-[10px] font-semibold ${priorityClass(ticketPriority)}`}>
                          {ticketPriority}
                        </span>
                      </td>

                      <td className="px-4 py-4">
                        <span className={`rounded-full px-2.5 py-1 text-[10px] font-semibold ${statusClass(ticketStatus)}`}>
                          {ticketStatus}
                        </span>
                      </td>
          ...
          <div className={`flex items-center justify-between border-t px-4 py-3 text-xs ${muted} ${isDark ? "border-[#2b3548]" : "border-[#e7dff3]"}`}>
            <span>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className={`rounded-lg border p-1.5 disabled:cursor-not-allowed disabled:opacity-40 ${control}`}
              >
                <ChevronLeft size={15} />
              </button>
