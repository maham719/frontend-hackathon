...
import { getAllTicketsService } from "../../tickets/services/ticket.service.js";
import { useTicket } from "../../tickets/context/TicketContext.jsx";
...
const AllTickets = () => {
  ...
  const handleDeleteTicket = async (ticketId) => {
    ...
    try {
      ...
      setTickets((currentTickets) => currentTickets.filter((ticket) => ticket._id !== ticketId));
    } catch (deleteError) {
      setError(deleteError.response?.data?.message || "Unable to delete this ticket right now.");
    }
  };
  ...
  const filteredTickets = useMemo(
    () =>
      tickets.filter((ticket) => {
        ...
        const searchable =
          `${ticket._id || ""} ${ticket.subject || ""} ${ticket.customer?.username || ""} ${ticket.customer?.email || ""}`.toLowerCase();
        return (
  ...
  return (
    ...
    >
      ...
      {loading ? (
        ...
        >
          <Clock3 size={18} /> Loading tickets...
        </div>
      ) : error ? (
        ...
        <div
          className={`overflow-hidden rounded-[18px] border ${isDark ? "border-[#2b3548]" : "border-[#e7dff3]"}`}
        >
            ...
            <table className="w-full min-w-[900px] text-left text-sm">
              ...
              >
                {visibleTickets.map((ticket) => {
                  ...
                  const ticketPriority = formatLabel(
                    ticket.priority,
                    priorityLabels,
                  );
                  return (
                    ...
                    >
                      ...
                      <td
                        className={`max-w-[240px] truncate px-4 py-4 font-medium ${heading}`}
                      >
                      ...
                      <td className="px-4 py-4">
                        <span
                          className={`rounded-full px-2.5 py-1 text-[10px] font-semibold ${priorityClass(ticketPriority)}`}
                        >
                          {ticketPriority}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-4">
                        <span
                          className={`rounded-full px-2.5 py-1 text-[10px] font-semibold ${statusClass(ticketStatus)}`}
                        >
                          {ticketStatus}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-4">
                        <button
                          ...
                          className={`rounded-lg p-2 transition-colors ${isDark ? "text-[#ff7777] hover:bg-[#ff5a5a1a]" : "text-[#d94c5c] hover:bg-[#ffe8eb]"}`}
                        >
                          <Trash2 size={16} />
                        </button>
                      </td>
          ...
          <div
            className={`flex items-center justify-between border-t px-4 py-3 text-xs ${muted} ${isDark ? "border-[#2b3548]" : "border-[#e7dff3]"}`}
          >
            <span>
